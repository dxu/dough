...
class SpatialHash {
  ...
  constructor(tileSize=10, comparator, testCollide, bucketHash) {
    ...
    // {
    //   gob_id: {
    //    buckets: [{bucketId: bucket_id, index: idx}...], represents the index of this gob within that bucket on this spatial hash's this.buckets!!
    //    bucketIndices: {
    //      [bucketId]: this.buckets' index of bucketId. Necessary for adjusting the buckets array!
    //    }
    //    gob: gob
    //   }
    // }
  ...
  getPossibleGobs(gob) {
    // TODO: can combine getBuckets with the reduce to halve the bucket calls
    ...
    //   for (let j=0; j<this.buckets[buckets[i]]; j++) {
    //     this.buckets[buckets[i]][j]

    //   }


    //   // console.log(Object.keys(this.buckets[buckets[i]]).length)
    //   let gobIds = Object.keys(this.buckets[buckets[i]])
    //   for (let j=0; j < gobIds.length; j++) {
    //     if (gob._id !== gobIds[j] && memo[gobIds[j]] == null) {
    //       memo[gobIds[j]] = this.buckets[buckets[i]][gobIds[j]]
    //     }
    //   }
    // }
    // console.log(memo)
    // return [].concat(...buckets.map((bucketId) => {
  ...
  addToOneBucket(gob, bucket) {
    ...
    // add gob to bucket
    this.buckets[bucket].push(gob)
    // this.buckets[bucket][gob._id] = gob
    // update items with the new bucket hash
    this.gobs[gob._id] = this.gobs[gob._id] || { buckets: [], bucketIndices: {}, gob }

    this.gobs[gob._id].buckets.push({
      bucketId: bucket,
      index: this.buckets[bucket].length - 1
    })
    this.gobs[gob._id].bucketIndices[bucket] = this.gobs[gob._id].buckets.length - 1
...
//     }
  ...
  removeAll(gob) {
    ...
    if (cachedGob = this.gobs[gob._id]) {
      // remove the gob from the bucket
      for (let i=0; i<cachedGob.buckets.length; i++) {
        let bucketInfo = cachedGob.buckets[i]

        // this.buckets[bucketInfo.bucketId].splice(bucketInfo.index, 1)

        console.log(this.buckets[bucketInfo.bucketId])
        // instead of splicing, swap it with the final item for performance

        if (this.buckets[bucketInfo.bucketId][bucketInfo.bucketId.length - 1] != null) {
          this.buckets[bucketInfo.bucketId][bucketInfo.index] = this.buckets[bucketInfo.bucketId][bucketInfo.bucketId.length - 1]

          this.buckets[bucketInfo.bucketId].pop()
          // the newly moved gob
          let movedGob = this.buckets[bucketInfo.bucketId][bucketInfo.index]
          console.log(movedGob, bucketInfo.bucketId.length, this.buckets[bucketInfo.bucketId])

          // update the location and other information of the gob that got moved
          let bucketIndexInGob = this.gobs[movedGob._id].bucketIndices[bucketInfo.bucketId]
          this.gobs[movedGob._id].buckets[bucketIndexInGob].index = bucketInfo.index
        }
...
//       }
  ...
  removeGob(gob) {
    ...
    for (let i=0; i<bucketsContainingGob.length; i++) {
      this.buckets[bucketsContainingGob[i].bucketId].splice([bucketsContainingGob[i].index], 1)
    }
