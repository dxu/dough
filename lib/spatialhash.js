...
class SpatialHash {
  ...
  getPossibleGobs(gob: Gob): Array<Array<Gob>> {
  ...
  _getBuckets(gob: Gob): Array<Array<Gob>> {
    const xVel: number = gob._velocity.x * Time.dts;
    const yVel: number = gob._velocity.y * Time.dts;

    const minX = gob._aabb[0].x;
    const maxX = gob._aabb[1].x;
    const minY = gob._aabb[0].y;
    const maxY = gob._aabb[3].y;
