...
export default class Game {
  ...
  updateCanvas() {
    let prevSec = this.seconds

    this.seconds = (Date.now() - this.startTime) / 1000

    this.fps = (this.fps * prevSec + 1) / this.seconds
    ...
    for (let gobId in this.gobs) {
      ...
      let prevX = this.gobs[gobId].position.x
      let prevY = this.gobs[gobId].position.y
      let prevWidth = this.gobs[gobId].width
      let prevHeight = this.gobs[gobId].height
      this.gobs[gobId].update()
      if (this.gobs[gobId].position.x != prevX ||
          this.gobs[gobId].position.y != prevY ||
          this.gobs[gobId].width != prevWidth ||
          this.gobs[gobId].height != prevHeight) {
        updated[gobId] = this.gobs[gobId]
