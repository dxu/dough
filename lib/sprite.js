...
export default class Sprite {
  ...
  constructor(options: {
    ...
    invariant(
      ...
      `[Sprite.js] No "frameStart" specified for ${gob.constructor.name}`,
    );
    invariant(
      this.frameEnd != null,
      `[Sprite.js] No "frameEnd" specified for ${gob.constructor.name} sprite`,
    );
    invariant(
      this.width != null,
      `[Sprite.js] No "width" provided for ${gob.constructor.name} sprite`,
    );
    ...
    if (instance != null) {
      if (instance.anchor) {
        invariant(
          instance.anchor.x != null,
          'instance anchor options must contain x',
        );
        invariant(
          instance.anchor.y != null,
