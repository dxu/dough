...
function updateRigidBody(body: Matter.Body, opts: GobRigidBodyProps) {
  const {
    mass,
    friction,
    fritionStatic,
  ...
  __init(scene: Scene, opts: GobOptions) {
    ...
    if (this.rigidbody || opts.rigidbody) {
      ...
      for (const key in RIGIDBODY_DEFAULTS) {
        console.log(optsRigidbody[key], this.rigidbody[key]);
        this.rigidbody[key] =
    ...
    if (this.collider != null) {
      ...
      if (!this.rigidbody) {
        ...
        for (const key in RIGIDBODY_DEFAULTS) {
          ...
          console.log('jj', key, this.collider.body[key]);
        }
