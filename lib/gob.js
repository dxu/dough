...
export default class Gob {
  ...
  currentSprite: string; // the string of the current loaded sprite for this instance
  collider: Collider;
  rigidbody: GobRigidBodyProps;
  sprite: GobSpriteOptions;
  static spritePaths: {[id: string]: string};
  ...
  __init(scene: Scene, opts: GobOptions) {
    ...
    if (this.rigidbody || opts.rigidbody) {
      ...
      this.rigidbody.mass = opts.rigidbody.mass || this.rigidbody.mass || 1;
      ...
      this.rigidbody.velocity = opts.rigidbody.velocity
        ? new Vector2(opts.rigidbody.velocity.x, opts.rigidbody.velocity.y)
  ...
  __initSprite() {
    ...
    if (this.sprite == null) {
      throw new Error(
        'Invalid game object created. No sprite properties provided',
      );
    }
    if (this.constructor.spritePaths == null) {
      throw new Error('Invalid game object created. No sprite paths provided');
    }
    // if no current sprite set, default to the first
    this.currentSprite =
      this.currentSprite || Object.keys(this.constructor.spritePaths)[0];
    if (
      this.scene.resources[
        Utils.getPixiResourceKey(this.constructor.name, this.currentSprite)
      ] == null
    ) {
      throw new Error(
        `No texture found for ${this.constructor.name}. Make sure it's being
        preloaded by ${this.scene.constructor.name}`,
      );
    }
    this.sprite.pixi = new PIXI.Sprite(
      this.scene.resources[
        Utils.getPixiResourceKey(this.constructor.name, this.currentSprite)
      ].texture,
    );
  ...
  __updateSprite(): void {
    if (this.sprite.pixi == null) {
      throw new Error(
        `No texture found for ${this.constructor.name}. Are you sure you're
