...
class Gob {
  ...
  constructor(game, opts) {
    ...
    this._nextPosition = new Vector2(0, 0)
    ...
    if (opts.position) {
      ...
      this._nextPosition.x = opts.position.x
      this._nextPosition.y = opts.position.y
    ...
    this._nextVelocity = new Vector2(this.velocity.x, this.velocity.y)
    console.log('first', this.velocity, this._nextVelocity)
  ...
  updateAABB() {
    // TL, BL
    this._aabb[0].x = this._aabb[3].x = this.position.x - this.width / 2
    // TR, BR
    this._aabb[1].x = this._aabb[2].x = this.position.x + this.width / 2

    // TL, BL
    this._aabb[0].y = this._aabb[1].y = this.position.y - this.height / 2
    // TR, BR
    this._aabb[2].y = this._aabb[3].y = this.position.y + this.height / 2

    this._edges = this._aabb.map((vertex, index) => {
      return Vector2.Subtract(this._aabb[(index + 1) % this._aabb.length], vertex)
    })
  ...
  _preUpdate() {
    this._debugData.previousPosition = this.position.clone()
  }
  ...
  _update() {
    ...
    this.velocity.x = this._nextVelocity.x
    this.velocity.y = this._nextVelocity.y

    // this.updatePosition(this.velocity.x * Time.dts, this.velocity.y * Time.dts)

    // this._nextVelocity.x = this.velocity.x + this.acceleration.x * Time.dts
    // this._nextVelocity.y = this.velocity.y + this.acceleration.y * Time.dts
    // console.log('next', this.velocity, this._nextVelocity)
  }
  ...
  _postUpdate() {
    ...
    this._nextPosition.x = this.position.x + this.velocity.x * Time.dts
    this._nextPosition.y = this.position.y + this.velocity.y * Time.dts
  }
  ...
  _debug() {
    ...
    this._debugData.outline.position.x += this.position.x - this._debugData.previousPosition.x
    this._debugData.outline.position.y += this.position.y - this._debugData.previousPosition.y
  }
