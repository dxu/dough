...
class Gob {
  ...
  calculateVertices(): void {
    ...
    this._vertices = this.relativeVertices.map((vertex) => {
      return Vector2.Sum(vertex, this._position);
    })
    // otherwise, update absolute vertices, and
    this.vertices = this.relativeVertices.map((vertex) => {
  ...
  updateNormals(): void {
    this._edges = this._vertices.map((vertex: Vector2, index: number): Vector2 => {
