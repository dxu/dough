...
export default class Gob {
  ...
  // the collection of collision groups
  collisionGroups: ?Array<string>;
  ...
  __init(scene: Scene, opts: GobOptions) {
    ...
    if (this.rigidbody) {
      this.rigidbody.collider.__init(this.scene.world);
    }
  ...
  __updatePostCollisionAttributes() {
    ...
    this.transform.position.x = this.rigidbody.collider.body.position[0];
    this.transform.position.y = this.rigidbody.collider.body.position[1];
  ...
  _debug(): void {
    ...
    if (this.rigidbody) {
      ...
      const path: Array<number, > = this.rigidbody.collider
        .getVertices()
        .reduce(
          (
            memo: Array<number>,
