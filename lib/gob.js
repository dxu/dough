...
class Gob {
  ...
  _id: number;
  _destroyed: bool;
  relativeVertices: Array<Vector2>; // specified relative to the position
  _vertices: Array<Vector2>;        // current, calculated according to relative
  vertices: Array<Vector2>;         // next, calculated according to relative
  _position: Vector2;  // current position
  position: Vector2;   // position at the next cycle
  _velocity: Vector2;  // current velocity
  velocity: Vector2;   // velocity at next cycle
  ...
  calculateVertices(): void {
    ...
    this._vertices = this.relativeVertices.map((vertex: Vector2): Vector2 => {
      return Vector2.Sum(vertex, this._position);
    });
    // otherwise, update absolute vertices, and
  ...
  calculateAABBFromVertices(): Array<Vector2> {
    ...
    let maxY: number = 0;
    // go through vertices
    this._vertices.map((vertex: Vector2): void => {
  ...
  updateNormals(): void {
    this._edges = this._vertices.map((
      vertex: Vector2,
      index: number,
    ): Vector2 => {
      return Vector2.Difference(
        this._vertices[(index + 1) % this._vertices.length], vertex,
      );
    });
  ...
  _setupDebug(game: Pool): void {
    ...
    const a: Array<number> = this._vertices.reduce((
      memo: Array<number>,
      vertex: Vector2,
      index: number,
      arr: Array<Vector2>,
  ...
  _debug(): void {
    ...
    this._debugData.outline.position.x +=
      this._position.x - this._debugData.previousPosition.x;
    this._debugData.outline.position.y +=
      this._position.y - this._debugData.previousPosition.y;
  }
