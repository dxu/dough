...
import type {Collider} from './colliders/interface.type';
...
export default class Gob {
  ...
  constructor(game: Pool, opts: GobOptions): void {
    ...
    this.position = new Vector2(opts.position.x, opts.position.y);
  ...
  __init() {
    ...
    if (this.collider) {
      this._body = this.collider(this);
      // both velocity and angular velocity are read-only, so we need to
      // initialize them here
      Matter.Body.setVelocity(
        this._body,
        Matter.Vector.create(this.velocity.x, this.velocity.y),
      );
  ...
  _updatePostCollisionAttributes() {
    this.position.x = this._body.position.x * Time.dts;
    this.position.y = this._body.position.y * Time.dts;
