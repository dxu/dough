...
class Gob {
  ...
  constructor(game, opts) {
    ...
    if (opts.position) {
      ...
      if (opts.width && opts.height) {
        ...
        let farthestLeft = opts.position.x - opts.width / 2
        let farthestRight = opts.position.x + opts.width / 2
        let farthestBottom = opts.position.y - opts.width / 2
        let farthestTop    = opts.position.y + opts.width / 2
        // even if we have the AABB, we should define a set of vertices
        // starting clockwise from top left

        this.vertices = [ new Vector2(-opts.width / 2, opts.height / 2),
                          new Vector2(opts.width / 2, opts.height / 2),
                          new Vector2(opts.width / 2, -opts.height / 2),
                          new Vector2(-opts.width / 2, -opts.height / 2) ]
  ...
  getAbsoluteVertices() {
    return this.vertices.map((vertex) => {
      return Vector2.add(vertex, this.position)
    })
  }
