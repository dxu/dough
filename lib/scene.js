...
export default class Scene {
  ...
  __collisionGroups: {};
  _createCollisionGroup(name: string) {
    this.__collisionGroups[name] = Math.pow(2, currentCollisionGroupID++);
  }
  ...
  createGob(opts: Object, GobClass: Class<Gob> = Gob): void {
    ...
    // for every collisiongroup defined on this, create a new collision group
    // if it doesn't exist
    gob.collisionGroups &&
      gob.collisionGroups.map(groupName => {
        this._createCollisionGroup(groupName);
        // register the collider function
      });
