...
function adjustPhysicsVelocity(gob1, gob2, normal) {
  ...
  const newv1t = Math.max(0, 1 - gob1.friction) * v1t

  // coefficient of restitution == bounciness
  // (v2n * (m2 - m1) + 2 * m1 * v1n) / (m1 + m2)
  const newv2n = gob2.bounce * (v2n * (m2 - m1) + 2 * m1 * v1n) / (m1 + m2)
  const newv2t = Math.max(0, 1 - gob2.friction) * v2t
  ...
  const newVelocityTangent1 = Vector2.Multiply(unitTangent, newv1t)
  const newVelocityTangent2 = Vector2.Multiply(unitTangent, newv2t)
