...
function adjustVelocity(surface, gob, surfaceGob) {
  ...
  let xMult
  if (Math.sign(gob.velocity.x) !== Math.sign(surfaceGob.velocity.x)) {
    // if they are going in opposite directions, always bounce away
    xMult = -1
  } else if (Math.abs(gob.velocity.x) > Math.abs(surfaceGob.velocity.x)) {
  ...
  let yMult
  if (Math.sign(gob.velocity.y) !== Math.sign(surfaceGob.velocity.y)) {
    // if they are going in opposite directions, always bounce away
    yMult = -1
  } else if (Math.abs(gob.velocity.y) > Math.abs(surfaceGob.velocity.y)) {
    // if this object caught up to the surface, always bounce away.
    yMult = -1
  }
  // shouldn't be able to have a collisino where one catches up to the other
...
export function movingSAT(gob1, gob2) {
  ...
  if (TFirst !== 0) {
    ...
    gob1._nextPosition.y = gob1.position.y + gob1.velocity.y  * (TFirst - 0.0001)
    gob1._nextPosition.x = gob1.position.x + gob1.velocity.x  * (TFirst - 0.0001)

    gob2._nextPosition.y = gob2.position.y + gob2.velocity.y  * (TFirst - 0.0001)
    gob2._nextPosition.x = gob2.position.x + gob2.velocity.x  * (TFirst - 0.0001)
...
// console.log('adjusting')
    ...
    // affecting
    gob1._nextVelocity = adjustVelocity(e2, gob1, gob2)
    gob2._nextVelocity = adjustVelocity(e1, gob2, gob1)
