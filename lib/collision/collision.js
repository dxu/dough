...
export function movingSAT(gob1, gob2) {
  ...
  // velocity
  const velocity = Vector2.Subtract(gob1.velocity, gob2.velocity)
  ...
  for (let i=0; i<normals1.length; i++) {
    // projection speed
    const speed = Vector2.Dot(normals1[i], velocity)
    let TFirst = 0
    let TLast = Infinity
    let min0 = Infinity
    let max0 = -Infinity
  ...
  console.log('colliding', Math.random())
  return true
