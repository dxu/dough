...
export function movingSAT(gob1, gob2) {
  console.log('in movingSat', gob1.position.y, gob1._previousPosition.y)
  // get the absolute vertices
  let vertices1 = gob1.getAbsoluteVertices()
  let vertices2 = gob2.getAbsoluteVertices()
  ...
  if (TFirst === 0) {
    // console.log('position', gob1.position.x, gob1.position.y, TElapsed)
    // console.log('position', gob1.position.x, gob1.position.y)

    // TODO: hardcoded fix to prevent it from phasing through
    // if (gob1.velocity.y !== 0) {
    //   gob1.position.y = gob1._previousPosition.y + 0.05 * (gob1.velocity.y > 0 ? -1 : 1)
    // }
    // if (gob1.velocity.x !== 0) {
    //   gob1.position.x = gob1._previousPosition.x + 0.05 * (gob1.velocity.x > 0 ? -1 : 1)
    // }
    // gob1.position.y = gob1._previousPosition.y - gob1.velocity.y * (TElapsed) * Time.dts
    // gob1.position.x = gob1._previousPosition.x - gob1.velocity.x * (TElapsed) * Time.dts
    // gob1.updatePosition(
    //   -gob1.velocity.x * (TElapsed) * Time.dts,
    //   -gob1.velocity.y * (TElapsed) * Time.dts
    // )
    // console.log('position', gob1.position.x, gob1.position.y)
  } else {
    // console.log('first', TFirst, gob1.velocity.y * (TFirst) * Time.dts, gob1._previousPosition.y)
    // it's going to collide with this next move. in this case
    gob1.position.y = gob1._previousPosition.y + gob1.velocity.y * (TFirst - 0.0001) * Time.dts
    gob1.position.x = gob1._previousPosition.x + gob1.velocity.x * (TFirst - 0.0001) * Time.dts
  }
