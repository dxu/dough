...
export default class Pool {
  ...
  constructor(renderer: Object, opts: Object): void {
    ...
    for (let i: number = 0; i <= renderer.view.width; i += this.tileSize) {
      this.grid.moveTo(i, 0);
      this.grid.lineTo(i, renderer.view.height);
    }
    for (let i: number = 0; i < renderer.view.height; i += this.tileSize) {
  ...
  createGob(opts: Object, GobClass: Class<Gob> = Gob): void {
    ...
    for (let i: number = 0; i < this.gobs.length; i++) {
  ...
  updateCanvas(): void {
    ...
    for (let i: number = 0; i < this.gobs.length; i++) {
      ...
      const prevX: number = this.gobs[i]._position.x;
      const prevY: number = this.gobs[i]._position.y;
