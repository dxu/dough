...
export default class Pool {
  ...
  constructor(opts: PoolOpts): void {
    ...
    this.loader = new Loader(this.loaded, this.audioContext);
  }
  ...
  preload(...gobs: Array<typeof Gob>) {
    ...
    spritePromise.then((resources) => {
      this.resources = resources;
    });
    Promise.all([spritePromise, audioPromise]).then(this.loaded).catch(
      (err) => console.log('Error loading audio sources: ', err)
    );
  }

  loaded = ([resources: Object, audioResources: Array<mixed>]: Array<mixed>) => {
