...
export default class Keyboard {
  ...
  // helper function for creating a event handler that executes all event handlers
  ...
  _registerCanvasEventListener(event) {
  ...
  _removeCanvasEventListener(event) {
  ...
  // register a handler for the particular event
  registerEventHandler(event, handler) {
    if (this.eventListeners[event] == null) {
      this._registeCanvasrEventListener(event)
    }
